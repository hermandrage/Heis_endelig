...
int check_for_order(int floor, int direction){
  for (int order_nr = 0 ; order_nr < nr_possible_orders ; ++order_nr ){
    if (que[order_nr][0] == floor && que[order_nr][1] == direction){// || que[order_nr][1] == ORDER_WITHOUT_DIRECTION)
    return order_nr;
    }
  }
  return -1;
...
int check_if_should_stop(int floor, int direction){
